...
export default function Favorites() {
  ...
  const removeFav = async (m) => {
    if (!window.confirm(`Xóa "${m.title}" khỏi yêu thích?`)) return;
    try {
      ...
      if (res.ok) {
        setFavs(favs.filter(x => x.id !== m.id));
        if (page > 1 && favs.length % ITEMS_PER_PAGE === 1) {
             setPage(page - 1);
        }
      }
    } catch (e) {}
  ...
  if (!user) return <div className="c-box"><h3>Vui lòng đăng nhập để xem danh sách yêu thích.</h3></div>;

  const totalPages = Math.ceil(favs.length / ITEMS_PER_PAGE);
  const currentFavs = favs.slice((page - 1) * ITEMS_PER_PAGE, page * ITEMS_PER_PAGE);
  ...
  return (
    <div className="fav-con">
      ...
      {favs.length === 0 ? (
        <div className="empty"><p>Chưa có phim nào.</p><Link to="/" className="btn">Tìm phim ngay</Link></div>
      ) : (
        <>
          <div className="grid">
            {currentFavs.map((m, i) => (
          ...
          {totalPages > 1 && (
            <div className="pagination">
              <button onClick={() => handlePageChange(page - 1)} disabled={page === 1} className="page-btn">&laquo; Prev</button>
              <span className="page-info">Page {page} of {totalPages}</span>
              <button onClick={() => handlePageChange(page + 1)} disabled={page === totalPages} className="page-btn">Next &raquo;</button>
            </div>
          )}
        </>
      )}
      ...
      <style>{`
        .fav-con{max-width:1200px;margin:0 auto;padding:30px 20px;min-height:80vh}
        .ti{margin-bottom:25px;border-bottom:2px solid #eee;padding-bottom:10px; color: var(--title)}
        .grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:25px}
        .card{background:#2c3e50;border-radius:8px;overflow:hidden;box-shadow:0 4px 10px rgba(0,0,0,.2);transition:transform .2s;display:flex;flex-direction:column}
        .card:hover{transform:translateY(-5px)}
        .img{width:100%;aspect-ratio:2/3;display:block;background:#000}
        .img img{width:100%;height:100%;object-fit:cover}
        .body{padding:12px;flex:1;display:flex;flex-direction:column;color:#fff}
        .name{font-weight:700;font-size:1rem;margin-bottom:5px;color:#fff;text-decoration:none;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}
        .meta{display:flex;justify-content:space-between;font-size:.85rem;color:#bdc3c7;margin-bottom:12px}
        .star{color:#f1c40f}
        .del{margin-top:auto;background:#e74c3c;color:#fff;border:none;padding:8px;border-radius:4px;cursor:pointer;font-weight:600}
        .del:hover{background:#c0392b}
        .empty{text-align:center;margin-top:50px;color:#7f8c8d}
        .btn{display:inline-block;margin-top:15px;padding:10px 25px;background:#3498db;color:#fff;text-decoration:none;border-radius:4px}
        
        .pagination { display: flex; justify-content: center; align-items: center; margin-top: 40px; gap: 15px; }
        .page-btn { padding: 8px 16px; background: #e74c3c; color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: bold; }
        .page-btn:disabled { background: #ccc; cursor: not-allowed; opacity: 0.6; }
        .page-info { font-weight: bold; color: var(--title); }
        .app.dark .ti, .app.dark .page-info { color: #fff; }
      `}</style>
    </div>
  );
}