...
export default function MovieDetail() {
  ...
  useEffect(() => {
    const fetchMovieDetail = async () => {
      ...
      try {
        ...
        if (!response.ok) {
            if (response.status === 401) throw new Error("Token hết hạn. Vui lòng cập nhật token mới.");
  ...
  const getPosterURL = (m) => {
    ...
    if (!imgSrc || imgSrc === 'N/A') return fallbackImage;

    if (imgSrc.includes('amazon.com') || imgSrc.includes('imdb.com')) {
        imgSrc = imgSrc.replace(/_V1_.*\.jpg$/, '_V1_.jpg');
    }
  ...
  const getRating = (m) => {
     if (m.ratings) {
         if (m.ratings.imDb) return m.ratings.imDb;
         if (m.ratings.theMovieDb) return m.ratings.theMovieDb;
     }
  ...
  const getOverview = (m) => {
     let text = "";
     if (m.plot_full) text = m.plot_full;
     else if (m.plot) text = m.plot;
     else if (m.short_description) text = m.short_description;
     else if (m.overview) text = m.overview;
  ...
  const getDirectors = (m) => {
      ...
      const list = Array.isArray(m.directors) ? m.directors : [m.directors];
      ...
      return list.map((d, index) => {
          ...
          return (
            <span key={index}>
              {index > 0 && ', '}
              {pid ? (
                <Link to={`/person/${pid}`} className="person-link">{name}</Link>
              ) : (
                <span>{name}</span>
              )}
            </span>
  ...
  const getActors = (m) => {
    ...
    if (Array.isArray(m.actors)) {
        actorsList = m.actors.filter(a => a);
    } else {
        actorsList = [m.actors];
    }

    return actorsList.slice(0, 15).map((a, index) => {
        const name = typeof a === 'object' ? (a.name || a.original_name) : a;
        const pid = typeof a === 'object' ? a.id : null;
        ...
        return (
            <span key={index}>
              {index > 0 && ', '}
              {pid ? (
                 <Link to={`/person/${pid}`} className="person-link actor-link">
                    {name}
                 </Link>
              ) : (
                 <span>{name}</span>
  ...
  if (!movie) return <div style={{color:'#fff', textAlign:'center', marginTop: 50}}>Không tìm thấy dữ liệu phim.</div>;
  ...
  return (
    <div className="movie-detail-container">
      ...
      <style>{`
        ...
        .overview-box::-webkit-scrollbar-track { background: rgba(255,255,255,0.05); }
        .overview-box::-webkit-scrollbar-thumb { background: #e74c3c; border-radius: 3px; }

        .detail-extra-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 20px; background: rgba(0,0,0,0.2); padding: 20px; border-radius: 10px; }
        ...
        .person-link { color: #fff; text-decoration: underline; transition: color 0.2s; }
        .person-link:hover { color: #e74c3c; }
        ...
        .actor-list { line-height: 1.6; color: #ddd; font-size: 0.95rem; }
        .actor-link { color: #ddd; text-decoration: none; border-bottom: 1px dotted #888; }
        .actor-link:hover { color: #e74c3c; border-bottom: 1px solid #e74c3c; }

        .back-btn { display: inline-block; padding: 10px 25px; background: #e74c3c; color: #fff; text-decoration: none; border-radius: 6px; font-weight: bold; }
        
        @media (max-width: 850px) { 
            .movie-content { flex-direction: column; align-items: center; padding: 20px; } 
            .detail-poster { width: 220px; } 
            .detail-info { text-align: center; } 
            .detail-meta { justify-content: center; } 
            .detail-extra-grid { text-align: left; } 
        }
      `}</style>
    </div>
  );
